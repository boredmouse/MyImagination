...
namespace RhoTools
{
    ...
    /// </summary>
    public static class CEditorGUILayout
    {
        static readonly Color SEED = new Color(0.361f, 0.875f, 0.608f);
        const float INCREMENT = 0.5f;
        static Dictionary<int, GUIStyle> _tabStyles;
        static Dictionary<int, GUIStyle> _boxStyles;
        ...
        {
            if (_tabStyles == null)
                _tabStyles = new Dictionary<int, GUIStyle>();
            if (!_tabStyles.ContainsKey(i))
            {
                ...
                if (_tabStyles.ContainsKey(i - 1))
                {
                    _tabStyles[i - 1].margin.right = 0;
                }
                _tabStyles.Add(i, tBoxStyle);
            }
        ...
        {
            if (_boxStyles == null)
                _boxStyles = new Dictionary<int, GUIStyle>();
            if (!_boxStyles.ContainsKey(i))
            {
                GUIStyle tBoxStyle = new GUIStyle(GUI.skin.box);
                Texture2D tTex = new Texture2D(1, 1);
                tTex.SetPixel(0, 0, GetColor(i));
        ...
        /// <param name="aCallback">Callback function called after object is dropped</param>
        /// <returns></returns>
        public static bool DropAreaGUI(string aText, float aHeight, System.Action<string> aCallback)
        {
            Event evt = Event.current;
            Rect drop_area = Box(aHeight, aText);
            switch (evt.type)
